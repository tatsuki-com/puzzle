...
const SIZE=5;
const source=fs.readFileSync(new URL('../www/stages.js',import.meta.url),'utf8');
const match=source.match(/window\.STAGES=(\[[\s\S]*\]);/);if(!match)throw new Error('Could not read stages from www/stages.js');
const stages=JSON.parse(match[1]);
const empty=()=>Array.from({length:SIZE},()=>Array(SIZE).fill(0));const same=(a,b)=>JSON.stringify(a)===JSON.stringify(b);
...
let failed=false;if(stages.length!==30){failed=true;console.error(`Expected 30 stages, found ${stages.length}`)}
...
if(failed)process.exit(1);console.log(`Validated ${stages.length} solvable stages.`);
